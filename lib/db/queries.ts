...
export async function getGradeByGradeId(gradeId: string) {
  try {
    const gradeWithSubjects = await db
      .select({
        grade: grades,
        subject: subjects,
      })
    ...
    const grouped = {
      ...gradeWithSubjects[0].grade,
      subjects: gradeWithSubjects.map((row) => row.subject).filter(Boolean),
    };
    return grouped;
  } catch (error) {
    console.log(error);
...
export async function getAllGradesByUser(userId: string) {
  try {
    const fetchedGrades = await db
      .select()
      .from(grades)
